...
*This cover letter template has been customized for the ${jobData.job_title} position at ${jobData.company_name}. Please personalize it with specific examples from your experience.*`;
  ...
  const downloadAsPDF = (content: string, filename: string) => {
    try {
      ...
      // Title from filename + header band
      const title = filename.replace('.pdf', '').replace(/_/g, ' ');
      pdf.setFillColor(18, 83, 102); // teal-dark
  ...
  const downloadAsRTF = (content: string, filename: string) => {
    try {
      ...
      // color table: black, teal
      rtfParts.push('{\\colortbl;\\red0\\green0\\blue0;\\red18\\green83\\blue102;}');
      rtfParts.push('\\margl720\\margr720\\margt720\\margb720');
      rtfParts.push('\\f0\\fs20');

      blocks.forEach((b) => {
        switch (b.type) {
          case 'h1':
            rtfParts.push(`\\par \\cf2 \\b \\fs30 ${escapeRTF(b.text)} \\b0 \\cf1 \\fs20`);
            // divider
            rtfParts.push('\\par {\\pard\\qr \\fs18 \\cf2 \\ul \\ulnone }');
            break;
          case 'h2':
  ...
  const downloadAsTXT = (content: string, filename: string) => {
    try {
      // Add simple structure with separators for readability
      const title = filename.replace(/\.(txt|pdf|docx)$/i, '').replace(/_/g, ' ');
      const sep = '\n' + '-'.repeat(64) + '\n';
      const enhanced = `# ${title}\n${sep}${content}\n${sep}Generated by Synergies4 Resume Customizer`;
